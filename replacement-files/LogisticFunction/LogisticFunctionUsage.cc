...
int main(int argc, char *argv[]) try
{
    ...
    ("setup", po::value<std::string>(), "set setup");
    ...
    if (!vm.count("setup"))
    {
        throw std::runtime_error("Setup is not specified.");
    }
    ...
    bool setup = vm["setup"].as<std::string>() == "true";
    ...
    if (!lbcrypto::Serial::SerializeToFile(
        ...
        std::move(std::get<lbcrypto::Ciphertext<lbcrypto::DCRTPoly>>(
            polycircuit::LogisticFunction<lbcrypto::DCRTPoly>(std::move(cc), std::move(inputC), std::move(setup)).evaluate())),
