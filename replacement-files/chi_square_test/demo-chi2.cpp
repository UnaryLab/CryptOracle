...
void RunChi2(const string &SNPDir, const string &SNPFileName,
		const string &pValue, const string &Runtime, const string &SampleSize, const string &SNPs, bool setup);
...
int main(int argc, char **argv) {
	...
	while ((opt = getopt_long(argc, argv, "S:s:p:r:N:M:z", long_options, &option_index)) != -1) {
		switch (opt)
		{
			case 'S':
				SNPDir = optarg;
			...
			default: /* '?' */
			  std::cerr<< "Usage: "<<argv[0]<<" <arguments> " <<std::endl
				   ...
				   << "  -z --setup true/false"  <<std::endl;
			  exit(EXIT_FAILURE);
		}
	}

	RunChi2(SNPDir, SNPFileName, pValue, Runtime, SampleSize, SNPs, setup);
...
void RunChi2(const string &SNPDir,
		const string &SNPFileName, const string &pValue, const string &Runtime, const string &SampleSize, const string &SNPs, const bool setup) {
