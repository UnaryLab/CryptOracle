...
public:
    ...
    explicit CIFAR10ImageClassification(const lbcrypto::CryptoContext<ElementType>& cc,
                                        const lbcrypto::Ciphertext<ElementType>& inputC, const bool setup)
        : m_cc(cc)
        , m_inputC(inputC)
        , setup(setup)
    { }
    ...
    {
        if (setup){
            return Ciphertext(std::move(m_inputC)); // Return the input ciphertext if input phase
        } 
        else {
...
private:
    ...
    const bool setup; // true for input phase, false for output phase
};
